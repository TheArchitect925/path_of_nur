...
- Hadith now has a real local ingestion/normalization/verification build pipeline:
  ...
  - Hadith collection detail ownership is now canonical at `/learn/hadith/collection/:collectionId`, with the older `/learn/hadith/subcategory/:subcategoryId` route retained as a compatibility alias because the underlying page has been collection-owned rather than taxonomy-owned
  - Hadith now also has a canonical large-library browse owner at `/learn/hadith/browse`, reusing the existing public corpus providers, shared narrowing controls, lazy rendering, and reader continuity lane handoff without replacing themes, collections, sources, or search
...
- Phase 51 Surah Summary enrichment hardening is complete:
  - the Quran Summary system now has a typed enrichment layer for surah themes, notable ayat, related prophets/events, cautious virtues/recitation notes, and reflection prompts instead of relying only on flat summary text
  - Surah Summary Detail now deep-links notable ayat through the shared Qur'an navigation helpers and renders enrichment sections only when grounded content exists
  - the enrichment model now also carries internal evidence/confidence categories so future editorial review and multilingual expansion can grow without mixing raw UI logic and content-safety decisions
...
- Phase 52 thematic Qur'an discovery is complete:
  - the canonical `/quran/topics` explorer now runs on a new curated theme-registry + resolved-surah discovery layer instead of the older loose topic graph alone, while preserving the existing route ownership and reader entrypoints
  - the main Qur'an hub now includes a dedicated Browse by Topic island alongside Quran Summary, using the shared Qur'an presentation system rather than a new page-specific visual stack
  - theme landing and theme detail flows now support curated categories, featured themes, search over aliases/prophets/events/surah names, related surah handoffs into Surah Summary Detail, and direct reader entry actions
  - thematic discovery now reuses the Phase 4 surah-enrichment model so normalized theme tags, related prophets/events, notable ayat, and related themes can power future recommendations without a parallel taxonomy system
...
- the existing Qur'an guided-path route family at `/quran/paths` has now been upgraded from a shallow continuity helper into a real Qur'an Pathways system:
  - `lib/features/learn/quran/domain/quran_guided_learning_path_models.dart` now defines typed pathway categories, richer stop kinds, ayah-reference metadata, and persisted per-path progress entries instead of only a light legacy path shell
  - `lib/features/learn/quran/data/seeded_quran_guided_learning_paths_data.dart` now seeds ten curated pathways such as patience, tawhid, Musa, mercy, gratitude, and hardship support, each with 4-stop guided journeys into the canonical surah summary, topic, and reader surfaces
  - `lib/features/learn/quran/application/quran_guided_learning_paths_provider.dart` now owns lightweight local progress and resume behavior for started paths, completed stops, next-stop resolution, and "continue pathway" surfacing while preserving the legacy provider/route names for compatibility
  - `lib/features/learn/quran/presentation/quran_learning_paths_page.dart` now provides a real landing page, pathway detail page, and stop-detail experience using the shared Qur'an feature header/cards instead of a thin static list, while the main Qur'an hub now exposes Pathways as a first-class island beside Summary and Browse by Topic
- the main Qur'an hub now has a first personalized Qur'an Companion layer instead of a generic "recommended" list:
  - `lib/features/learn/quran/domain/quran_hub_recommendation_models.dart` now defines typed companion recommendation, rationale, destination, and context-snapshot models so personalization logic is explainable and route-safe instead of widget-owned ad hoc conditionals
  - `lib/features/learn/quran/application/quran_hub_recommendations_provider.dart` now builds a lightweight rules-based companion stack from real local signals such as continue-reading state, recent surahs, active pathway progress, selected Qur'an focus, daily companion context, time-of-day, and Friday context, with deduped primary/secondary suggestions plus evergreen fallbacks for low-data users
  - `lib/features/learn/presentation/pages/quran_app_hub_page.dart` now renders that data as a localized "Your Qur'an Companion" section with one elevated primary card, compact secondary cards, rationale chips, and visible resume progress instead of a flat list of generic hub suggestions
- the existing Qur'an reflections owner at `/quran/reflections` has now been upgraded from a light ayah-save list into a richer saved-insights system without creating a second notes stack:
  - `lib/features/learn/quran/domain/quran_reflection_entry.dart` now stores richer source-aware metadata such as source ids/labels, optional surah/theme/pathway ownership, route back-link metadata, and favorite state while remaining backward-compatible with older saved daily-ayah entries
  - `lib/features/learn/quran/application/quran_reflections_provider.dart` still owns the local reflection store, but now supports source-aware upserts, favorite toggles, entry lookup by id, and the stronger identity rules needed for surah/theme/pathway reflection capture
  - `lib/features/learn/quran/presentation/quran_reflections_page.dart` now provides both the saved-insights library and a dedicated reflection-detail/edit owner with search, filter chips, favorites, source back-linking, and safe missing-entry fallbacks
  - Surah Summary Detail, Theme Detail, Pathway Detail, and Pathway Stop Detail now all use a shared reflection-capture helper to open the same calm reflection composer with pre-bound source context instead of inventing page-local note flows
- the Qur'an Summary list now also has a first surah-background art pipeline prepared for the Top 10 rollout without faking assets:
  - `lib/features/learn/quran/domain/quran_surah_summary_background_spec.dart`, `lib/features/learn/quran/data/quran_surah_summary_background_registry.dart`, and `lib/features/learn/quran/application/quran_surah_summary_background_resolver.dart` now define typed surah-art specs, exact external-generation prompts, stable asset paths, and null-safe fallback resolution for the first ten surahs
  - `lib/features/learn/quran/presentation/widgets/quran_surah_summary_card_background.dart` and `lib/features/learn/quran/presentation/quran_summary_page.dart` now layer future surah-art assets inside the existing summary cards with clipped bounds, low-opacity image treatment, and a readability-preserving overlay, while safely rendering nothing until real `.webp` files are added
  - the external prompt pack and optimization guidance now live in `docs/quran_summary_top_10_surah_background_art_prompts_2026-03-31.md`, and the current ready-asset set is intentionally empty because image generation was not available in this session
- the shared appearance system now includes a real Midnight Manuscript theme mode rather than only the default / easy-read / dark variants:
  - `lib/core/theme/app_theme.dart` now defines `AppThemeMode.midnightManuscript` plus richer appearance tokens for border/divider/input/Qur'an emphasis, success tone, and Makki/Madani chip colors so the new palette flows through the shared glass architecture instead of page-local overrides
  - `lib/features/profile/presentation/settings_page.dart` now surfaces Midnight Manuscript as a persisted first-class theme option with compact preview tiles in the existing appearance picker, reusing the same settings persistence path as other themes
  - `lib/core/theme/app_backgrounds.dart` and `lib/shared/widgets/global_background.dart` now provide a central theme-aware atmosphere resolver so Midnight Manuscript can tint the user's selected wallpaper, fall back to tokenized gradients safely, and keep glass cards readable without page-level background forks
...
- 2026-04-11: Added the first Hadith narrator-info surface under `lib/features/learn/hadith/` with a canonical `/learn/hadith/narrator/:narratorId` route, safe canonical narrator resolution from inconsistent raw narrator strings, a new narrator profile page with library stats plus curated/fallback narrator summaries, and clickable `Narrated by` metadata tiles in the reader that deep-link into the narrator page when a narrator can be resolved confidently.
