...
void main() {
  Future<(LocalStore, AppDatabase)> setUpStores(
  ...
  test(
    'prayer repository migrates legacy day records and is idempotent',
    () async {
  ...
  test(
    'ocean repository migrates legacy blob and keeps dedup-relevant events stable',
    () async {
      final (localStore, database) = await setUpStores(<String, Object>{
      ...
      expect(
        first.awardedEligibilityKeys,
        contains('prayer_completed|prayer|dhuhr|2026-03-14'),
      );
  ...
  test(
    'repositories handle malformed or empty legacy data without crashing',
    () async {
      final (localStore, database) = await setUpStores(<String, Object>{
        'worship.prayer.2026-03-14': '[]',
        'worship.dhikr': '"bad"',
      ...
      expect(
        () => prayerRepository.readDailyRecords('2026-03-14'),
        returnsNormally,
      );
