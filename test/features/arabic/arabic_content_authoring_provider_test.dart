...
void main() {
  test(
    'kids content authoring units cover letters words phrases and bridge items',
    () async {
  ...
  test(
    'adult content authoring units include shared modules and short surahs',
    () async {
      final container = await makeTestContainer();
      addTearDown(container.dispose);
  ...
  test(
    'pack compositions resolve to valid unit ids for both audiences',
    () async {
      final container = await makeTestContainer();
      addTearDown(container.dispose);
