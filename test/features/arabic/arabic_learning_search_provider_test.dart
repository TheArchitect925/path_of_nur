...
void main() {
  test('kids search finds letters and locked beginner words safely', () async {
    ...
    final letter = results.firstWhere(
      (item) => item.type == ArabicLearningSearchItemType.letter && item.id == 'kids_letter_ba',
    );
    final word = results.firstWhere(
      (item) => item.type == ArabicLearningSearchItemType.word && item.id == 'kids_word_bab',
    );
  ...
  test('kids continue filter falls back to the first calm start target', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);
  ...
  test('kids packs filter returns lesson-pack results through existing routes', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);
  ...
  test('adult search routes letters and words to adult-specific targets', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);
    ...
    expect(letterResults.first.target.kind, ArabicLearningRouteTargetKind.adultLesson);
    ...
    expect(word.target.kind, ArabicLearningRouteTargetKind.adultBeginnerWords);
  ...
  test('adult packs filter returns module-backed lesson packs safely', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);
