...
void main() {
  ...
  test('selects only verified strong duas by default', () async {
    ...
    expect(
      candidates.every((candidate) => candidate.item.isVerifiedStrong),
      isTrue,
    );
  });
  ...
  test(
    'recent history avoids repeating the same dua when alternatives exist',
    () async {
      final dataset = await loadDataset();
      ...
      final first = service.getBestMatch(
        dataset: dataset,
        context: baseContext,
      );
  ...
  test(
    'provider exposure returns a trusted prompt with overridden context',
    () async {
