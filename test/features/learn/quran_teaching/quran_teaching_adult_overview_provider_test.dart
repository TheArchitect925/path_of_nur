...
void main() {
  test('adult overview starts with calm start state and browse targets', () async {
    ...
    expect(summary.primaryTarget.kind, ArabicLearningRouteTargetKind.adultLesson);
    expect(summary.browseLettersTarget?.kind, ArabicLearningRouteTargetKind.adultModule);
  ...
  test('adult overview shows in-progress state after lesson completion', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);
  ...
  test('adult overview falls into review state after all lessons are complete', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);
