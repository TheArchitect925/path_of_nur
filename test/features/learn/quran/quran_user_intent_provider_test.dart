...
void main() {
  test(
    'quran user intent restores a persisted memorization focus safely',
    () async {
  ...
  test(
    'guided-path focus prefers the current continue path when available',
    () async {
      final container = await makeTestContainer();
      addTearDown(container.dispose);
