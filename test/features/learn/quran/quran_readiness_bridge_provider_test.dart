...
void main() {
  test('quran readiness bridge resolves curated snippet text from real ayahs', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);

    final snippets = container.read(quranReadinessBridgeSnippetsProvider);
    final bismillah = snippets.firstWhere((item) => item.id == 'bismillah_bridge');
    ...
    expect(
      qulhuwa.hints.first.adultTarget?.lessonId,
      'qalqalah_intro',
    );
  });
  ...
  test('adult quran readiness summary continues forward after one snippet', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);

    container
        .read(quranReadinessBridgeProgressProvider(ArabicLearningAudience.adult).notifier)
  ...
  test('bridge progression advances into the next level after first level snippets', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);

    final notifier = container.read(
      quranReadinessBridgeProgressProvider(ArabicLearningAudience.adult).notifier,
    );
