...
void main() {
  ...
  test(
    'manual backup state is reflected for local-first tvOS resilience',
    () async {
  ...
  test(
    'pending sync has higher priority than remote backup readiness',
    () async {
      final store = await storeWith(<String, Object>{});
      final db = AppDatabase.inMemory();
      db.execute(
  ...
  test(
    'route snapshots preserve enabled tvOS routes and parity counts',
    () async {
      final store = await storeWith(<String, Object>{});
      final db = AppDatabase.inMemory();
      ...
      expect(
        snapshot.routeSnapshots
            .firstWhere((route) => route.routePath == '/quran')
            .parityBackedSectionCount,
        4,
      );
      expect(
        snapshot.routeSnapshots
            .firstWhere((route) => route.routePath == '/learn')
            .offlineReady,
        isTrue,
      );
    },
