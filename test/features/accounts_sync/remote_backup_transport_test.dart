...
void main() {
  ...
  test(
    'remote backup upload persists remote backup metadata for google account',
    () async {
  ...
  test(
    'remote restore preview compares local and remote backup timestamps',
    () async {
      final payload =
      ...
      expect(preview!.comparisonSummary.domainComparisons, isNotEmpty);
      expect(preview.importPreview.profileNames, contains('Traveler'));
    },
  );

  test(
    'restore comparison warns on account mismatch and disables safe merge',
    () async {
      final container = await makeTestContainer();
      addTearDown(container.dispose);
      final controller = container.read(
      ...
      final localValidation = await container
          .read(backupRepositoryProvider)
          .validateImportPayload(payload: localPayload, encrypted: false);
      final remoteValidation = await container
          .read(backupRepositoryProvider)
          .validateImportPayload(payload: localPayload, encrypted: false);
      ...
      expect(
        summary.availableDecisionModes,
        isNot(contains(RestoreDecisionMode.mergeSafeDomains)),
      );
    },
  );

  test(
    'merge-safe remote restore merges prayer data and keeps remote replace-only settings',
    () async {
      final container = await makeTestContainer(
        overrides: <Override>[
          accountsSyncPackageInfoProvider.overrideWith(
      ...
      final structured =
          (remoteMap['structuredDataByProfile'] as Map<String, dynamic>);
      ...
      (remoteMap['metadata'] as Map<String, dynamic>)['exportedAtIso'] =
      ...
      final scopedContainer = await makeTestContainer(
        seed: Map<String, Object>.from(
          container.read(localStoreProvider).dumpAll(),
        ),
      ...
      final restoredStructured = restoredDatabase.exportStructuredData(
        profileId,
      );
      final prayers = (restoredStructured['prayerRecords'] as List)
      ...
      expect(
        prayerIds,
        containsAll(<String>['2026-03-24:fajr', '2026-03-24:maghrib']),
      );
    },
  );
}
