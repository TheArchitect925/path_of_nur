...
void main() {
  test('profile switching restores isolated local snapshots without leakage', () async {
    ...
    await store.setJsonMap('worship.prayer.${LocalStore.todayKey()}', <String, dynamic>{
    ...
    await store.setJsonMap('settings.profile', <String, dynamic>{'theme': 'teal'});
    await store.setJsonMap('worship.prayer.${LocalStore.todayKey()}', <String, dynamic>{
    ...
    expect(
      store.getJsonMap('worship.prayer.${LocalStore.todayKey()}')?['fajr']?['status'],
      'completed',
    );
  ...
  test('backup payload contains expected top-level sections and imports safely', () async {
    ...
    await store.setJsonMap('settings.profile', <String, dynamic>{'language': 'en'});
    ...
    await target.read(accountsSyncControllerProvider.notifier).importBackup(
          payload: payload,
          encrypted: false,
    ...
    expect(imported.profileSnapshots[imported.profiles.first.profileId], isNotEmpty);
  });
  ...
  test('structured backup restores prayer, dhikr, and ocean state without cross-profile leakage', () async {
    ...
    final profileA = source.read(accountsSyncControllerProvider).activeProfileId!;
    source.read(prayerLogRepositoryProvider).saveDailyRecords(
          '2026-03-14',
          <DailyPrayerRecord>[
            for (final prayer in PrayerName.values)
              DailyPrayerRecord(
                prayer: prayer,
                status:
                    prayer == PrayerName.fajr ? PrayerStatus.completed : PrayerStatus.pending,
                completedAtIso:
                    prayer == PrayerName.fajr ? '2026-03-14T05:45:00Z' : null,
              ),
          ],
        );
    source.read(dhikrRepositoryProvider).saveState(
          selectedPresetId: 'subhanallah',
          target: 33,
          currentCount: 11,
        );
    source.read(oceanDropServiceProvider).awardDrop(
          actionType: oceanActionPrayerCompleted,
          sourceModule: oceanSourcePrayer,
          referenceId: 'fajr',
          metadata: const <String, dynamic>{'timestamp': '2026-03-14T05:45:00Z'},
        );
      ...
      <DailyPrayerRecord>[
        for (final prayer in PrayerName.values)
          DailyPrayerRecord(
            ...
            completedAtIso:
                prayer == PrayerName.asr ? '2026-03-14T16:30:00Z' : null,
          ),
      ],
    );
    ...
    await target.read(accountsSyncControllerProvider.notifier).switchProfile(
    ...
    final pendingOutbox = target.read(syncOutboxRepositoryProvider).pendingCount(importedProfile.profileId);
  ...
  test('profile switching keeps structured prayer, dhikr, and ocean state isolated', () async {
    final container = await makeTestContainer();
    addTearDown(container.dispose);
    final controller = container.read(accountsSyncControllerProvider.notifier);
          ...
          <DailyPrayerRecord>[
            for (final prayer in PrayerName.values)
              DailyPrayerRecord(
                prayer: prayer,
                status:
                    prayer == PrayerName.maghrib ? PrayerStatus.completed : PrayerStatus.pending,
                completedAtIso:
                    prayer == PrayerName.maghrib ? '2026-03-14T18:45:00Z' : null,
              ),
          ],
        );
    container.read(dhikrRepositoryProvider).saveState(
          selectedPresetId: 'alhamdulillah',
          target: 33,
          currentCount: 21,
        );
    container.read(oceanDropServiceProvider).awardDrop(
          actionType: oceanActionPrayerCompleted,
          sourceModule: oceanSourcePrayer,
          referenceId: 'maghrib',
          metadata: const <String, dynamic>{'timestamp': '2026-03-14T18:45:00Z'},
        );
    ...
    expect(
      container.read(prayerLogRepositoryProvider).readDayEntries('2026-03-14')[PrayerName.maghrib],
      isNull,
    );
    ...
    expect(container.read(syncOutboxRepositoryProvider).pendingCount(profileB), 0);

    await controller.switchProfile(profileA);

    expect(
      container.read(prayerLogRepositoryProvider).readDayEntries('2026-03-14')[PrayerName.maghrib]?.status,
