...
void main() {
  test(
    'ocean drops deduplicate same prayer reward on the same logical day',
    () async {
  ...
  test(
    'ocean drops persist and rebuild deterministically after reload',
    () async {
      ...
      expect(
        state.events.map((event) => event.sourceModule).toSet(),
        containsAll(<String>{oceanSourcePrayer, oceanSourceLearn}),
      );
    },
  );

  test(
    'free dhikr carry awards only when 100-count threshold is crossed',
    () async {
      final container = await makeTestContainer();
      addTearDown(container.dispose);
      final service = container.read(oceanDropServiceProvider);
