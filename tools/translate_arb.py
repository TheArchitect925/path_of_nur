...
def split_translatable_entries(
    arb: dict[str, object],
...
def make_client(api_key: str) -> Any:
    try:
        ...
        raise RuntimeError(
            ...
            "Install it with `python3 -m pip install openai` before running this script."
        ) from exc
...
def call_translation_model(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        except Exception as exc:  # pragma: no cover - API retry behavior
            if attempt == max_retries:
                raise
            sleep_seconds = attempt * 2
            print(
                f"[retry {attempt}/{max_retries}] {exc} — sleeping {sleep_seconds}s",
                file=sys.stderr,
            )
...
def translate_language(
    ...
    for index, batch in enumerate(chunks, start=1):
        print(f"  - batch {index}/{len(chunks)} ({len(batch)} keys)")
        user_prompt = build_user_prompt(batch)
        translated_batch = call_translation_model(
            client,
            model,
            system_prompt,
            user_prompt,
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to source ARB, e.g. lib/l10n/app_en.arb",
    )
    parser.add_argument(
        "--outdir",
        required=True,
        help="Output directory for translated ARBs",
    )
    parser.add_argument(
        "--languages",
        nargs="+",
