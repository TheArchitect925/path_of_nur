# Phase 5 Turkish Localization Backlog

Date: 2026-03-23

## Current blocker

- `app_tr.arb` is not in the same state as the Urdu, Arabic, and German files.
- Current parity audit:
  - English keys: `9025`
  - Turkish keys: `3387`
  - Missing keys: `5640`
  - Extra keys: `2`
  - Placeholder mismatches in the current file: `520`
- This is not a small QC patch. It is a large-scale locale build-out.

## Why this was not force-completed in one pass

- A safe production-ready Turkish pass needs a staged translation workflow with placeholder protection and QA.
- The current repo Turkish file is too incomplete to honestly claim “done” after a quick patch.
...
1. Run a staged Turkish merge script that:
   - copies metadata from `app_en.arb`
   - preserves structurally valid existing Turkish strings
...
3. Prioritize Turkish high-traffic screens first if full-locale completion is split across multiple passes.
