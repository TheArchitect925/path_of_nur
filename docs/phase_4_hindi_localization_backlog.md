# Phase 4 Hindi Localization Backlog

Date: 2026-03-23

## Current blocker

- `app_hi.arb` is not in the same state as the Urdu, Arabic, and German files.
- Current parity audit:
  - English keys: `9025`
  - Hindi keys: `3498`
  - Missing keys: `5530`
  - Extra keys: `3`
  - Placeholder mismatches in the current file: `485`
- This is not a small QC patch. It is a large-scale locale build-out.

## Why this was not force-completed in one pass

- A safe production-ready Hindi pass needs a staged translation workflow with placeholder protection and QA.
- The current repo Hindi file is too incomplete to honestly claim “done” after a quick patch.
...
1. Run a staged Hindi merge script that:
   - copies metadata from `app_en.arb`
   - preserves structurally valid existing Hindi strings
...
3. Prioritize Hindi high-traffic screens first if full-locale completion is split across multiple passes.
